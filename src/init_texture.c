...
/*   Updated: 2020/01/27 04:56:28 by deladia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rt.h"

int			*fill_texture_for_object(char *texture_path, int *texture_pixels, int *texture_param)
{
	SDL_Surface		*image;
	SDL_Surface		*tmp;
	int				*texture_pixels;
	...
	if (texture_pixels == NULL)
	{
		...
		ft_memcpy(texture_pixels, (int *)image->pixels, image->w * image->h * sizeof(int));
		texture_param[0] = -1; //начало
